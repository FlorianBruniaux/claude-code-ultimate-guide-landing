 ...
 * Reuses EXAMPLES from examples/index.html when available
 */
...
(function() {
    ...
    function buildIndex() {
        ...
        // Templates from EXAMPLES (if available)
        if (window.EXAMPLES) {
            Object.entries(window.EXAMPLES).forEach(([category, data]) => {
                if (data.files) {
                    data.files.forEach(file => {
                        items.push({
                            id: `t-${category}-${file.name}`,
                            type: 'template',
                            title: file.name,
