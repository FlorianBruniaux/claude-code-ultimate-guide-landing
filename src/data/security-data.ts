...
export interface SecurityStats {
    readonly skills_scanned: number;
    readonly flawed_pct: number;
    readonly critical_risk: number;
    readonly critical_pct: number;
    readonly malicious_payloads: number;
    readonly cves_tracked: number;
    readonly malicious_skills: number;
    readonly exposed_servers: number;
    readonly hardcoded_secrets_pct: number;
    readonly still_live: number;
}

export interface MaliciousAuthor {
...
export interface MaliciousSkill {
    readonly name: string;
...
export interface Campaign {
    ...
    readonly amos_skills?: number;
    readonly outlier_skills?: number;
    readonly malware?: string;
    readonly delivery?: string[];
    readonly c2_ips?: string[];
    readonly c2_ip?: string;
    readonly c2_port?: number;
    readonly targets?: string[];
    readonly categories?: Record<string, number>;
    readonly outliers?: Record<string, string[]>;
    readonly skills_scanned?: number;
    readonly findings?: Record<string, number>;
    readonly known_malicious_authors?: string[];
...
export interface SecurityData {
    readonly meta: SecurityMeta;
    readonly stats: SecurityStats;
    readonly malicious_authors: readonly MaliciousAuthor[];
    readonly malicious_skills: readonly MaliciousSkill[];
    readonly cve_database: readonly CveEntry[];
    readonly campaigns: readonly Campaign[];
    readonly attack_techniques: readonly AttackTechnique[];
    readonly scanning_tools: readonly ScanningTool[];
    readonly sources: readonly SecuritySource[];
    readonly minimum_safe_versions: Readonly<Record<string, string>>;
}
